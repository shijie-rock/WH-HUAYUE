var initPositionTable = function(tableId,aoColumns,reqData,appPath,colIdx,transeDataIndexStr) {
	$("#"+tableId).dataTable({
			"bFilter": false,//去掉搜索框
		    ...
		    "aoColumnDefs": [ {
		        ...
		        "fnCreatedCell": function (nTd, sData, oData, iRow, iCol) {
		        		...
		        		if('1'==oData.FREEZE_TAG){ //stop
		        			buttonHtml="<button type='button' class='btn btn-success btn-xs' href='javascript:void(0);' onclick='optConfirm(\"start\",\""+ oData.POSITION_NAME+"\",\"" + oData.POSITION_ID+"\")'>启用</button>"
		        		}
		        		else{
		        			buttonHtml="<button type='button' class='btn btn-warning btn-xs' href='javascript:void(0);' onclick='optConfirm(\"stop\",\""+ oData.POSITION_NAME+"\",\"" + oData.POSITION_ID+"\")'>停用</button>"
		        		}	
		        		$(nTd).addClass("optd").css("text-align","center").html(
		        		"<button type='button' class='btn btn-primary btn-xs' href='javascript:void(0);' onclick='editPosition(\"" + oData.POSITION_ID+"\",\""+ oData.POSITION_NAME+"\")'>编辑</button>&nbsp;&nbsp;"+buttonHtml
		        		+"&nbsp;&nbsp;<button type='button' class='btn btn-danger btn-xs' href='javascript:void(0);' onclick='optConfirm(\"delete\",\""+ oData.POSITION_NAME+"\",\"" + oData.POSITION_ID+"\")'>删除</button>"
		        		);
		        }
		      },
		      {
			      ...
			      },
		      {
			        "aTargets": [1], //
			        "fnCreatedCell": function (nTd, sData, oData, iRow, iCol) {
			        	return $(nTd).html("<a class='data-detail' data-toggle='tooltip' data-placement='top' title='点击查看详情' href='javascript:void(0)' onclick='detail(\"" + oData.POSITION_ID+"\",\"" + oData.POSITION_NAME+"\")'>"+oData.POSITION_CODE+"</a>"); //dicTranse
			          }
			      },
		      {
			        "aTargets": [5], //
...
//			        	return $(nTd).html("<span style='font-size:14px;color:#7F7F7F'>"+oData.CODE+"-"+oData.TYPE_CODE+"</span>" +""); //dicTranse
			      ...
			      },
				  {
				    	"aTargets": [3], //
				    	  "fnCreatedCell": function (nTd, sData, oData, iRow, iCol) {
				    		  return $(nTd).html(parseNull2Empty(oData.POSITION_DESC));
				      }
				  },
			      {
			    	  "aTargets": [4], //
			    	  "fnCreatedCell": function (nTd, sData, oData, iRow, iCol) {
			    		  return $(nTd).html(parseNull2Empty(oData.POSITION_ADDRESS));
			    	  }
			      }
		      ],
