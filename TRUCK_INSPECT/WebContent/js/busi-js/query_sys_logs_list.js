var initSysOptLogsTable = function(tableId,aoColumns,reqData,appPath,colIdx,transeDataIndexStr) {
	$("#"+tableId).dataTable({
			"bFilter": false,//去掉搜索框
		    ...
		    "aoColumnDefs": [ 
		        ...
		        "fnCreatedCell": function (nTd, sData, oData, iRow, iCol) {
		        		...
		        		if('1'==oData.FREEZE_TAG){ //stop
		        			buttonHtml="<button type='button' class='btn btn-success btn-xs' href='javascript:void(0);' onclick='optConfirm(\"start\",\""+ oData.INS_GROUP_NAME+"\",\"" + oData.INS_GROUP_ID+"\")'>启用</button>"
		        		}
		        		else{
		        			buttonHtml="<button type='button' class='btn btn-warning btn-xs' href='javascript:void(0);' onclick='optConfirm(\"stop\",\""+ oData.INS_GROUP_NAME+"\",\"" + oData.INS_GROUP_ID+"\")'>停用</button>"
		        		}	
		        		$(nTd).addClass("optd").css("text-align","center").html(
		        		"<button type='button' class='btn btn-primary btn-xs' href='javascript:void(0);' onclick='editInsGroup(\"" + oData.INS_GROUP_ID+"\")'>编辑</button>&nbsp;&nbsp;"+buttonHtml
		        		+"&nbsp;&nbsp;<button type='button' class='btn btn-danger btn-xs' href='javascript:void(0);' onclick='optConfirm(\"delete\",\""+ oData.INS_GROUP_NAME+"\",\"" + oData.INS_GROUP_ID+"\")'>删除</button>"
		        		);
		        }
		      },*/
		      {
			      ...
			      },
			  {
			    	  ...
			    	  "fnCreatedCell": function (nTd, sData, oData, iRow, iCol) {
				        	if(oData.OPT_NAME!='--'){
				        		return $(nTd).html("<a class='data-detail' data-toggle='tooltip' data-placement='top' title='点击查看详情' href='javascript:void(0)' onclick='detailEmp(\"" + oData.OPT_MEMBER_ID+"\")'>"+oData.OPT_NAME+"</a>"); //dicTranse
				        	}else{
				        		return $(nTd).html(oData.OPT_NAME); //dicTranse
				        	}
			    	  }
			  },
		      {
			        "aTargets": [4], //
			        "fnCreatedCell": function (nTd, sData, oData, iRow, iCol) {
			        	if(oData.OPT_CONTENT!='--'){
			        		return $(nTd).html("<a class='data-detail' data-toggle='tooltip' data-placement='top' title='点击查看详情' href='javascript:void(0)' onclick='detail(\"" + oData.LOGS_ID+"\")'>"+oData.OPT_CONTENT+"</a>"); //dicTranse
			        	}else{
			        		return $(nTd).html(oData.OPT_CONTENT); //dicTranse
			        	}
			          }
			      },
			  /*
