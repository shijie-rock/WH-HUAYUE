var initMemberTable = function(tableId,aoColumns,reqData,appPath,colIdx,transeDataIndexStr) {
	$("#"+tableId).dataTable({
			"bFilter": false,//去掉搜索框
		    ...
		    "aoColumnDefs": [ {
		        ...
		        "fnCreatedCell": function (nTd, sData, oData, iRow, iCol) {
		        		...
		        		if('1'==oData.FREEZE_TAG){ //stop
		        			buttonHtml="<button type='button' class='btn btn-success btn-xs' href='javascript:void(0);' onclick='optConfirm(\"start\",\""+ oData.MEMBER_NAME+"\",\"" + oData.MEMBER_ID+"\")'>启用</button>"
		        		}
		        		else{
		        			buttonHtml="<button type='button' class='btn btn-warning btn-xs' href='javascript:void(0);' onclick='optConfirm(\"stop\",\""+ oData.MEMBER_NAME+"\",\"" + oData.MEMBER_ID+"\")'>停用</button>"
		        		}	
			      ...
			      },
		      {
			      ...
			      },
			   {
			    	"aTargets": [2], //
			    	  "fnCreatedCell": function (nTd, sData, oData, iRow, iCol) {
			    		  return $(nTd).html(parseNull2Empty(oData.MOBILE)+"<br/>"
			    				  +parseNull2Empty(oData.EMAIL)); //
			    	  }
			      },
			   {
			    	 "aTargets": [3], //
			    	  "fnCreatedCell": function (nTd, sData, oData, iRow, iCol) {
			    		  return $(nTd).html(dicTranse(oData.JOB_TITEL_TYPE)); //
			    	  }
			      },
			  {
			    	 "aTargets": [5], //
			    	  "fnCreatedCell": function (nTd, sData, oData, iRow, iCol) {
			    		  return $(nTd).html(parseNull2Empty(oData.CAN_LOGIN_SYS)+"<br/>"
			    				  +parseNull2Empty(oData.IS_INSPACTOR)); //
			    	  }
			      },
		      {
			        "aTargets": [8], //
