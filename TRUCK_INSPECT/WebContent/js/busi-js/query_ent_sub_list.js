var initEntSubTable = function(tableId,aoColumns,reqData,appPath,colIdx,transeDataIndexStr) {
	$("#"+tableId).dataTable({
			"bFilter": false,//去掉搜索框
		    ...
		    "aoColumnDefs": [ {
		        ...
		        "fnCreatedCell": function (nTd, sData, oData, iRow, iCol) {
		        		...
		        		if('1'==oData.FREEZE_TAG){ //stop
		        			buttonHtml="<button type='button' class='btn btn-success btn-xs' href='javascript:void(0);' onclick='optConfirm(\"start\",\""+ oData.CHECK_ENT_CODE+"\",\"" + oData.ENT_SUB_ID+"\")'>启用</button>"
		        		}
		        		else{
		        			buttonHtml="<button type='button' class='btn btn-warning btn-xs' href='javascript:void(0);' onclick='optConfirm(\"stop\",\""+ oData.CHECK_ENT_CODE+"\",\"" + oData.ENT_SUB_ID+"\")'>停用</button>"
		        		}	
		        		$(nTd).addClass("optd").css("text-align","center").html(
		        		"<button type='button' class='btn btn-primary btn-xs' href='javascript:void(0);' onclick='editEntMid(\"" + oData.ENT_SUB_ID+"\")'>编辑</button>&nbsp;&nbsp;"+buttonHtml
		        		+"&nbsp;&nbsp;<button type='button' class='btn btn-danger btn-xs' href='javascript:void(0);' onclick='optConfirm(\"delete\",\""+ oData.CHECK_ENT_CODE+"\",\"" + oData.ENT_SUB_ID+"\")'>删除</button>"
		        		+"<br/>"
		        		+"<button type='button' class='btn btn-default btn-xs btn-line2large' href='javascript:void(0);' onclick='subEntMapSubObj(\""+ oData.CHECK_ENT_CODE+"\",\"" + oData.ENT_SUB_ID+"\",\"" + oData.CHECK_ENT_NAME+"\")'>检查项目</button>"
		        		);
		        }
		      },
		      {
			      ...
			      },
			      {
			        "aTargets": [1], //
			        "fnCreatedCell": function (nTd, sData, oData, iRow, iCol) {
			        	return $(nTd).html("<a class='data-detail' data-toggle='tooltip' data-placement='top' title='点击查看详情' href='javascript:void(0)' onclick='detail(\"" + oData.ENT_SUB_ID+"\")'>"+oData.CHECK_ENT_CODE+"</a>"); //dicTranse
			          }
			      },
			      {
			    	  "aTargets": [2], //
			    	  "fnCreatedCell": function (nTd, sData, oData, iRow, iCol) {
			    		  return $(nTd).html(parseNull2Empty(oData.CHECK_ENT_F_CODE)+"<br/>"+parseNull2Empty(oData.MID_NAME)); //dicTranse
			    	  }
			      },
			      {
			    	  "aTargets": [5], //
...
//				        "aTargets": [4], //data dic
//				        "fnCreatedCell": function (nTd, sData, oData, iRow, iCol) {
//				        	return $(nTd).html(parseNull2Empty(dicTranse(oData.CHECK_ENT_TYPE_CODE))); //dicTranse
//				          }
//				      },
		      {
			        "aTargets": [6], //
